...
class ControllerCheckoutOnepagecheckout extends Controller
{
    ...
    public function index()
    {
        ...
        if (isset($this->session->data['payment_address']['postcode'])) {
            $data['payment_address']['postcode'] = $this->session->data['payment_address']['postcode'];
        }
        if (isset($this->session->data['payment_address']['country_id'])) {
            $data['payment_address']['country_id'] = $this->session->data['payment_address']['country_id'];
        }
        if (isset($this->session->data['shipping_address']['country_id'])) {
            $data['shipping_address']['country_id'] = $this->session->data['shipping_address']['country_id'];
        }
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            if ($this->validate_form()) {
                ...
                if (isset($this->request->post['country_id'])) {
                    ...
                    $this->session->data['shipping_address']['country'] = $this->request->post['country_id'];
                    $order_data['shipping_country'] = $this->request->post['country_id'];
                }
            ...
            foreach ($method_data as $i => $shipping_method)
                foreach ($shipping_method['quote'] as $shipping_method2) {
                    ...
                    if (isset($shipping_method2['cost']))
                        $data['shippig_methods'][$i]['cost'] = $this->currency->format($shipping_method2['cost'], $this->session->data['currency']);
            ...
            // Set method
            // $this->session->data['payment_method']['code'] = 'oplata';
    ...
    public function validate_form()
    {
        ...
        if ((utf8_strlen(trim($this->request->post['email'])) < 6) || (utf8_strlen(trim($this->request->post['email'])) > 32)) {
            $data['error']['email'] = $this->language->get('error_email');
        }
        if ((utf8_strlen(trim($this->request->post['country_id'])) < 1) || (utf8_strlen(trim($this->request->post['country_id'])) > 200)) {
